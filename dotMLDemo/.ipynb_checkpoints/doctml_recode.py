...
cubes = load_cubes("F:\\fabric\\dotMLDemo\\data_cube.yml")

query = {
    "fields": ["orders.booking_date_month", "orders_items.quantity"],
...
group by {group_expr}
    ...
    for cube in cubes:
        ...
        where_expr = ""
        # Juno - I donot think below is required as where was handled in previous dimension CTE
        if cube.get('always_where_conditions') is not None and len(cube.get('always_where_conditions')) > 0:
